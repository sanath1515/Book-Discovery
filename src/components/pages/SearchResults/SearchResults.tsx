...
import Pagination from "../../molecules/PaginationBox/Pagination";
...
const SearchResult: React.FC<SearchResultProps> = ({
	...
	const onCategoryChange = (event: any) => {
		...
		if (event.target.value === "1") {
			...
			let changedCategory = categories.find(
				categoryBook => categoryBook.id == event.target.value
			);
			let newBooks = booksData.filter(
				(book: any) => book.category == changedCategory?.name
			);
			setCategory(changedCategory!.name!);
			let newBooksAfterSort = newBooks.sort((book1, book2) => {
				return compareObjects(book1, book2, sort);
			});
			setCategoryBooks(newBooksAfterSort);
		}
	...
	const filteredSearchBooks = searchBooks.filter(
		searchByAuthor
			? searchText !== ""
				? (book: bookProp) =>
						(book.title &&
	...
	const onPageChange = (unknown: any, pageNumber: number) => {
		getPage(pageNumber);
	};

	let searchValue = category !== "" ? category : searchText;
	let dropdownCategory = categories.find(
		categoryBook => categoryBook.name === searchValue
	);
	return (
